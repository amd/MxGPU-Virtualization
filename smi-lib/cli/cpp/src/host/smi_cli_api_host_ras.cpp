...
extern AMDSMI_GET_FABRIC_CPER_ENTRIES host_amdsmi_get_fabric_cper_entries;
							   ...
							   int error_count = 1)
{
	...
	bool is_ifoe = (std::memcmp(&notify_type, &ifoe_guid, 16) == 0);

	switch (error_severity) {
	case AMDSMI_CPER_SEV_NON_FATAL_UNCORRECTED:
		prefix = is_ifoe ? "fabric-linkdown-" : "uncorrected-";
		break;
	case AMDSMI_CPER_SEV_NON_FATAL_CORRECTED:
		prefix = is_ifoe ? "fabric-linkup-" : "corrected-";
		break;
...
static void collect_cper_entries_from_buffer(char* cper_data, amdsmi_cper_hdr_t** cper_hdrs,
					     uint64_t entry_count, int gpu_id,
					     std::vector<CperEntryInfo>& entries_info)
{
...
int AmdSmiApiHost::amdsmi_get_cper_entries_command(Arguments arg, std::string& out)
{
	...
	uint64_t cursor[AMDSMI_MAX_DEVICES] {0};
	uint64_t fabric_cursor[AMDSMI_MAX_DEVICES] {0};
