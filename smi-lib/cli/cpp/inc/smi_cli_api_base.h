...
public:
	...
	virtual int amdsmi_get_engine_process_command(uint64_t processor, Arguments arg,
			std::string &out_string, int &proc_num, int gpu_id) override;
	virtual int amdsmi_get_version_command(uint64_t processor_bdf, Arguments arg, std::string& out) override;
	...
	virtual int amdsmi_reset_gpu_command(uint64_t processor_bdf, Arguments arg) override;
