...
#ifndef NAVI32_DIAG_DATA_H
#define NAVI32_DIAG_DATA_H

#define NAVI32_HOST_REGISTERS_DUMP_BLK_SIZE	(4 * AMDGV_DIAG_DATA_ALIGNMENT)
/* PSP block division */
#define NAVI32_PSP_SNAPSHOT_DUMP_BLK_SIZE	(128 * AMDGV_DIAG_DATA_ALIGNMENT)
#define NAVI32_PSP_TRACE_LOG_DUMP_BLK_SIZE	(4 * AMDGV_DIAG_DATA_ALIGNMENT)
#define NAVI32_INTER_STRUCT_SIZE		(1 * AMDGV_DIAG_DATA_ALIGNMENT)

/* Static check for the host driver memory size
 * Snapshot logs such as NAVI32_HOST_REGISTERS_DUMP_BLK_SIZE
 ...
 * memory to hold data
 */
#define NAVI32_TOTAL_SIZE	(NAVI32_PSP_SNAPSHOT_DUMP_BLK_SIZE + \
				NAVI32_PSP_TRACE_LOG_DUMP_BLK_SIZE + \
				NAVI32_INTER_STRUCT_SIZE)
#if (NAVI32_TOTAL_SIZE > AMDGV_DIAG_DATA_ASIC_MEM_SIZE)
#error The AMDGV_DIAG_DATA_ASIC_MEM_SIZE is too small to allocate all blocks.
#endif

/* Get the inter struct offset */
#define NAVI32_INTER_STRUCT_OFFSET \
	(adapt->diag_data.asic_buff.vaddr ? \
	(uint8_t *)adapt->diag_data.asic_buff.vaddr + \
	(AMDGV_DIAG_DATA_ASIC_MEM_SIZE - \
	NAVI32_INTER_STRUCT_SIZE) : NULL)

/* Fills in the memory block for the given block */
#define NAVI32_DIAG_DATA_FILL_MEM_BLK(mem_blk, c_addr, BLOCK) { \
	mem_blk.vaddr = (void *)AMDGV_DIAG_DATA_ALIGN(c_addr); \
	mem_blk.size =  NAVI32_ ## BLOCK ## _BLK_SIZE; \
...
extern struct amdgv_diag_data_dump_reg navi32_diag_data_regs[];
extern uint32_t navi32_diag_data_regs_count;
