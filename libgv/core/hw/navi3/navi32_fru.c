...
static int navi32_fru_ping_rm(struct amdgv_adapter *adapt)
{
	int ret;
	...
	ret = adapt->pp.pp_funcs->i2c_eeprom_xfer(adapt, I2C_PORT_SVD_SCL, &msg, 1);
	adapt->pp.is_test_smu = false;

	return ret;
}

static int navi32_fru_read_header(struct amdgv_adapter *adapt, uint16_t header_address,
 ...
 */
static int navi32_fru_read_product_info(struct amdgv_adapter *adapt, uint16_t data_addr,
...
static void navi32_fru_copy_field(struct amdgv_adapter *adapt, enum navi32_fru_product_order id,
...
int navi32_fru_get_product_info(struct amdgv_adapter *adapt)
{
	...
	if (!adapt->product_info.valid) {
		AMDGV_WARN("product info is invalid, skip get product info\n");
		return AMDGV_FAILURE;
	}
	 ...
	 */
	ret = navi32_fru_read_header(adapt, NAVI32_FRU_PRODUCT_INFO_HEADER, &addrptr);
	if (ret)
		goto clear;
	...
	for (i = 0; i < FRU_PRODUCT_MAX; i++) {
		...
		if (i == FRU_PRODUCT_ASSET_TAG) {
			addrptr += NAVI32_FRU_PRODUCT_INFO_ASSET_TAG_SIZE;
			continue;
		}

		ret = navi32_fru_read_product_info(adapt, addrptr, cmd_buff, &data_size);
		if (ret)
			goto clear;
		...
		navi32_fru_copy_field(adapt, i, string, data_size);
