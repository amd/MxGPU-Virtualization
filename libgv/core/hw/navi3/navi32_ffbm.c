...
static int navi32_ffbm_request_access(struct amdgv_adapter *adapt, bool enable)
{
	...
	wait_ret = amdgv_wait_for_register(adapt, SOC15_REG_OFFSET_NAME(GC, 0, regGCUTCL2_FFBM_ACCESS_CNTL), GCUTCL2_FFBM_ACCESS_CNTL__TLB_ACCESS_GRANT_MASK,
		GCUTCL2_FFBM_ACCESS_CNTL__TLB_ACCESS_GRANT_MASK, AMDGV_TIMEOUT(TIMEOUT_STATUS_REG), check_mask, AMDGV_WAIT_FLAG_FORCE_DELAY);
	if (!wait_ret)
		wait_ret = amdgv_wait_for_register(adapt, SOC15_REG_OFFSET_NAME(MMHUB, 0, regMMUTCL2_FFBM_ACCESS_CNTL), MMUTCL2_FFBM_ACCESS_CNTL__TLB_ACCESS_GRANT_MASK,
...
static int navi32_ffbm_invalidate_tlb(struct amdgv_adapter *adapt, struct amdgv_ffbm_pte_block *pteb, bool flush)
{
	...
	for (current_gpa = pteb->gpa;
		...
		if (flush) {
			wait_ret = amdgv_wait_for_register(adapt, SOC15_REG_OFFSET_NAME(GC, 0, regGCUTCL2_FFBM_INVALIDATE_RESPONSE), GCUTCL2_FFBM_INVALIDATE_RESPONSE__FLUSHTYPE_INVALIDATE_ACK_MASK,
			...
			if (!wait_ret)
				wait_ret = amdgv_wait_for_register(adapt, SOC15_REG_OFFSET_NAME(MMHUB, 0, regMMUTCL2_FFBM_INVALIDATE_RESPONSE), MMUTCL2_FFBM_INVALIDATE_RESPONSE__FLUSHTYPE_INVALIDATE_ACK_MASK,
			...
			wait_ret = amdgv_wait_for_register(adapt, SOC15_REG_OFFSET_NAME(GC, 0, regGCUTCL2_FFBM_INVALIDATE_RESPONSE), GCUTCL2_FFBM_INVALIDATE_RESPONSE__NON_FLUSHTYPE_INVALIDATE_ACK_MASK,
			...
			if (!wait_ret)
				wait_ret = amdgv_wait_for_register(adapt, SOC15_REG_OFFSET_NAME(MMHUB, 0, regMMUTCL2_FFBM_INVALIDATE_RESPONSE), MMUTCL2_FFBM_INVALIDATE_RESPONSE__NON_FLUSHTYPE_INVALIDATE_ACK_MASK,
