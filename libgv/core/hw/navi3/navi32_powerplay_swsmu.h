 ...
 *
 *  File Name      smu_13_0_0_pptable.h
 *  Project        AMD SMU Software IP Module
 *
 *  Description    SMU PowerPlay Table header file
 */
...
struct smu_13_0_0_powerplay_table {
    ...
    uint8_t padding;                        //Padding 1 byte to align table_size offset to 6 bytes
    ...
    uint32_t reserve[45];                   //Zero filled field reserved for future use

    struct smu_13_0_0_overdrive_table overdrive_table;

    uint8_t padding1;                       //Padding 1 byte to align to a 32-byte boundary
    PPTable_Navi32_t smc_pptable;
};
