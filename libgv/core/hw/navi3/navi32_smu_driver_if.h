...
#ifndef SMU13_DRIVER_IF_NAVI32_H
#define SMU13_DRIVER_IF_NAVI32_H

// *** IMPORTANT ***
// PMFW TEAM: Always increment the interface version on any change to this file
#define SMU13_DRIVER_IF_VERSION  0x40

//Increment this version if SkuTable_t or BoardTable_t change
#define PPTABLE_VERSION 0x2C
...
#define THROTTLER_TEMP_HOTSPOT_GCD_BIT 2
#define THROTTLER_TEMP_HOTSPOT_MCD_BIT 3
...
#define THROTTLER_TEMP_VR_USR_BIT      9
...
#define THROTTLER_TDC_USR_BIT          15
...
#define THROTTLER_GFX_DVO_BIT          22
#define THROTTLER_COUNT                23
...
#define FW_DSTATE_USR_ULV_BIT               12
#define FW_DSTATE_MALL_FLUSH_BIT            13
#define FW_DSTATE_SOC_PSI_BIT               14
#define FW_DSTATE_USR_PSI_BIT               15
#define FW_DSTATE_USRCP_DS_BIT              16
...
#define FW_DSTATE_USR_LOW_PWR_MODE_EN_BIT   22
...
typedef enum {
  ...
  I2C_CONTROLLER_THROTTLER_VR_USR,
...
typedef struct {
  ...
  uint32_t               Spare[10];
  uint32_t               MmHubPadding[8]; // SMU internal use. Adding here instead of external as a WA
} OverDriveTable_t;
...
typedef struct {
  ...
  //PLL 6
  //MCD clocks, assumed all MCDs will be the same. For NV33, only UCLK is applicable
  ...
  uint16_t InitUsr; // In Mv(Q2) not applicable in NV33
...
typedef struct {
  ...
  uint16_t     UlvVoltageOffsetUsr; // In mV(Q2). ULV offset used in either USR_ULV(part of FW_DSTATE)
  ...
  uint8_t      FclkDpmUsrPstates[NUM_FCLK_DPM_LEVELS]; // USR P-state ID associated with each FCLK DPM state.
  uint16_t     FclkDpmVddUsr[NUM_FCLK_DPM_LEVELS]; // mV(Q2) Vset USR voltage associated with each FCLK DPM state.
  uint16_t     FclkDpmUsrSpeed[NUM_FCLK_DPM_LEVELS]; //USR speed associated with each FCLK DPM state
  ...
  AvfsFuseOverride_t SocAvfsFuseOverride[AVFS_DIE_COUNT];

  DroopInt_t        dBtcGbSoc[AVFS_DIE_COUNT];            // GHz->V BtcGb

  LinearInt_t       qAgingGb[AVFS_DIE_COUNT];          // GHz->V

  QuadraticInt_t    qStaticVoltageOffset[AVFS_DIE_COUNT]; // GHz->V

  AvfsDcBtcParams_t DcBtcSocParams[AVFS_DIE_COUNT];
...
typedef struct {
  ...
  uint32_t     VoltageTelemetryRatio[SVI_PLANE_COUNT]; // This is used for VDDIO  Svi2 Div Ratio WA. It has 16 fractional bits (Q16.16)
...
//disable VCN router and skip asserting GC reset during pnp
#define SRIOV_GC_RESET_VCN_ROUTER                   0x55
