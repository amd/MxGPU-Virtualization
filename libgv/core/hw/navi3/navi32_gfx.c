...
void navi32_gfx_unhalt_gpu_state(struct amdgv_adapter *adapt)
{
	...
	if (adapt->flags & AMDGV_FLAG_USE_PF) {
		...
		tmp = REG_SET_FIELD(tmp, CP_ME_CNTL, CE_PIPE0_RESET, 0);
		tmp = REG_SET_FIELD(tmp, CP_ME_CNTL, CE_PIPE1_RESET, 0);

		tmp = REG_SET_FIELD(tmp, CP_ME_CNTL, ME_INVALIDATE_ICACHE, 0);
		tmp = REG_SET_FIELD(tmp, CP_ME_CNTL, PFP_INVALIDATE_ICACHE, 0);
		tmp = REG_SET_FIELD(tmp, CP_ME_CNTL, CE_INVALIDATE_ICACHE, 0);
	}
