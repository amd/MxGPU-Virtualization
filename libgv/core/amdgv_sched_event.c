...
static const char *amdgv_event_name(uint32_t event)
{
	...
	case AMDGV_EVENT_SCHED_RAS_EVENT:
		return "SCHED_RAS_EVENT";
	case AMDGV_EVENT_SCHED_VF_RAS_REMOTE_CMD:
		return "AMDGV_EVENT_SCHED_VF_RAS_REMOTE_CMD";
	...
	case AMDGV_EVENT_SCHED_VF_REQ_GPU_INIT_XCHG_REGION:
		return "VF_REQ_GPU_INIT_XCHG_REGION";
...
static int amdgv_sched_psp_set_mb_int(struct amdgv_adapter *adapt, int idx_vf, bool enable)
{
	...
	if (NEED_SWITCH_TO_PF(adapt)) {
		...
		if (ret) {
			amdgv_put_log(AMDGV_PF_IDX, AMDGV_LOG_SCHED_WORLD_SWITCH_FAIL, 0);
			return ret;
		}
	}
					   ...
					   bool wake_event_thread)
{
	...
	// Do not use vf_idx other than PF index to perform certain actions
	if (amdgv_sched_event_is_event_invalid(adapt, idx_vf, event_id, sched_block)) {
		AMDGV_WARN("Event %s request from %s for %s is pushed with incorrect parameters\n",
	...
	if ((unsigned char)(wptr + 1) == rptr) {
		amdgv_put_log(AMDGV_PF_IDX, AMDGV_LOG_SCHED_EVENT_QUEUE_FULL,
				AMDGV_LOG_DATA_32_32(rptr, wptr));
		oss_spin_unlock_irq(adapt->sched.queue_lock);
		return AMDGV_FAILURE;
	}
...
static void amdgv_sched_notify_vf_full_access(struct amdgv_adapter *adapt, uint32_t idx_vf)
{
	...
	msg_data[1] = (uint32_t)GET_VF_TABLE_OFFSET_BY_ID(adapt, idx_vf, INITD_H);
	/* checksum key */
	msg_data[2] = 0;
...
static int amdgv_sched_handle_req_gpu_reset(struct amdgv_adapter *adapt, uint32_t idx_vf)
{
	...
	if (AMDGV_PF_IDX != idx_vf) {
		amdgv_vbios_upload_image_to_vf(adapt, idx_vf);
		amdgv_vfmgr_update_pf2vf_message(adapt, idx_vf);
	}
...
static void amdgv_sched_notify_vf_init_data_ready(struct amdgv_adapter *adapt, uint32_t idx_vf)
{
	...
	if (adapt->array_vf[idx_vf].xchg.vf_crit_region == GPU_CRIT_REGION_V3) {
		msg_data[0] = MB_RES_MSG_GPU_INIT_DATA_READY;
		msg_data[1] = GPU_CRIT_REGION_V3;
		msg_data[2] = amdgv_vfmgr_calc_crit_region_pool_size(adapt, idx_vf);
		msg_data[3] = 0;
	} else if (adapt->array_vf[idx_vf].xchg.vf_crit_region == GPU_CRIT_REGION_V2) {
		msg_data[0] = MB_RES_MSG_GPU_INIT_DATA_READY;
...
{
	/* Host does not know where VF exchange region is located. Delay
	 * the copy until guest provides the info. */
	if (!adapt->use_vf_sysmem_xchg) {
		...
		if (adapt->fw_load_type == AMDGV_FW_LOAD_BY_GIM_PHASE_2)
			amdgv_host_upload_fw_to_vf(adapt, idx_vf);
	...
	if (amdgv_uniras_enabled(adapt)) {
		amdgv_ras_mgr_vf_lifespan_init(adapt, idx_vf);
	} else {
	...
	uint32_t idx_vf)
{
	...
	msg_data[0] = MB_RES_RAS_REMOTE_CMD_READY;
	/* checksum key */
	msg_data[2] = 0;
	amdgv_mailbox_send_msg(adapt, idx_vf, msg_data, MAILBOX_DATA_LEN_3, true);
	amdgv_mailbox_wait_trn_msg_ack(adapt);
...
static int amdgv_sched_handle_ras_poison_consumption(struct amdgv_adapter *adapt, struct amdgv_sched_event *event)
{
	...
	if (amdgv_uniras_enabled(adapt)) {
		amdgv_ras_mgr_handle_consumer_interrupt(adapt,	event);
	} else {
		if (adapt->ecc.poison_consumption)
			ret = adapt->ecc.poison_consumption(adapt, event);
	}

	amdgv_vfmgr_update_pf2vf_message(adapt, event->idx_vf);
					   ...
					   uint32_t guard_id)
{
	...
	if (amdgv_guard_add_active_event(adapt, idx_vf, guard_id) == AMDGV_EVENT_OVERFLOW) {
		amdgv_put_log(idx_vf, AMDGV_LOG_SCHED_IGNORE_EVENT, event_id);
		return AMDGV_FAILURE;
	}
						     ...
						     uint32_t idx_vf)
{
	...
	if (amdgv_uniras_enabled(adapt))
		amdgv_ras_mgr_update_ras_ecc(adapt);
	else
...
static int amdgv_sched_handle_vf_req_ras_cmd(struct amdgv_adapter *adapt,
				struct amdgv_sched_event *event)
{
	int ret;

	ret = amdgv_ras_remote_cmd_handler(adapt, event);
	if (ret) {
		amdgv_sched_notify_vf_fail(adapt, event->idx_vf);
		return AMDGV_FAILURE;
	}

	amdgv_sched_notify_vf_req_ras_cmd_ready(adapt, event->idx_vf);
	return 0;
}
...
static void amdgv_sched_notify_vf_xchg_region_ready(struct amdgv_adapter *adapt, uint32_t idx_vf)
{
	uint32_t msg_data[MAILBOX_DATA_LEN_4];

	if (adapt->array_vf[idx_vf].xchg.vf_crit_region == GPU_CRIT_REGION_V3) {
		msg_data[0] = MB_RES_MSG_REQ_GPU_INIT_XCHG_REGION_READY;
		msg_data[1] = GPU_CRIT_REGION_V3;
		msg_data[2] = (uint32_t) GET_VF_TABLE_OFFSET_BY_ID(adapt, idx_vf, INITD_H);
		msg_data[3] = GET_VF_TABLE_SIZE_KB_BY_ID(adapt, idx_vf, INITD_H);
...
static void amdgv_sched_init_xchg_region(struct amdgv_adapter *adapt, uint32_t idx_vf,
					uint64_t gpa_base, uint32_t size)
{
	...
	if (amdgv_vfmgr_init_xchg_region(adapt, idx_vf, gpa_base, size)) {
		amdgv_sched_notify_vf_fail(adapt, idx_vf);
		return;
	}

	if (amdgv_vfmgr_init_crit_region(adapt, idx_vf)) {
	...
	amdgv_sched_notify_vf_xchg_region_ready(adapt, idx_vf);
}
...
static int amdgv_sched_handle_req_gpu_fini(struct amdgv_adapter *adapt, uint32_t idx_vf)
{
	...
	if (is_avail_vf(idx_vf)) {
		...
		AMDGV_WARN("Attempt to request fini for inactive VF, skip\n");
		return AMDGV_FAILURE;
	}

	if (amdgv_uniras_enabled(adapt))
		amdgv_ras_mgr_clear_vf_auto_list(adapt, idx_vf);
...
static int amdgv_sched_handle_rel_gpu_fini(struct amdgv_adapter *adapt, uint32_t idx_vf)
{
	...
	if (adapt->sched.num_vf_per_gfx_sched == 1 &&
		...
		adapt->pp.pp_funcs->prepare_vf_unload(adapt, idx_vf);
					 ...
					 struct amdgv_sched_event *event)
{
	...
	switch (event->id) {
	case AMDGV_EVENT_REQ_GPU_INIT:
		amdgv_mmsch_config_vf(adapt, event->idx_vf);
		...
		if (adapt->array_vf[event->idx_vf].xchg.crit_region_realloc_needed) {
			AMDGV_INFO("Re-allocating crit region for VF%d after bad page FLR\n", event->idx_vf);
			adapt->array_vf[event->idx_vf].xchg.crit_region_realloc_needed = false;

			if (amdgv_vfmgr_init_crit_region(adapt, event->idx_vf)) {
				AMDGV_ERROR("Critical region re-allocation failed for VF%d, setting to cond_avail\n",
								event->idx_vf);
		...
		amdgv_irqmgr_update_virtualized_interrupt(adapt, event->idx_vf);
		break;
	case AMDGV_EVENT_REQ_GPU_RESET:
		/* enable VF FB access */
					 ...
					 struct amdgv_sched_event *event)
{
	...
	if (mb_status.status != AMDGV_MB_STATUS_OK) {
		...
		amdgv_put_log(event->idx_vf, AMDGV_LOG_FW_MAILBOX_ERROR,
						AMDGV_LOG_DATA_16_16_32((uint16_t)event->idx_vf, (uint16_t)mb_status.status, mb_status.fw_id));
	}
	...
	if (!one_vf) {
		...
		if (!adapt->lock_world_switch)
			amdgv_sched_context_one_time_loop(adapt, event->idx_vf);
	}
...
static void amdgv_sched_exit_full_access_timeout(struct amdgv_adapter *adapt, uint32_t vf_idx)
{
	...
	amdgv_put_log_ext(idx_vf, AMDGV_LOG_DRIVER_FULL_ACCESS_TIMEOUT, idx_vf,
						adapt->sched.start_time_full_access, oss_get_time_stamp());
	...
	if (mb_status.status != AMDGV_MB_STATUS_OK) {
		...
		amdgv_put_log(idx_vf, AMDGV_LOG_FW_MAILBOX_ERROR,
						AMDGV_LOG_DATA_16_16_32((uint16_t)idx_vf, (uint16_t)mb_status.status, mb_status.fw_id));
	}
...
static int amdgv_sched_toggle_full_access_for_debug(struct amdgv_adapter *adapt, int idx_vf, bool enable)
{
	...
	if (enable) {
		...
		ret = amdgv_gfx_rlc_safe_mode(adapt, enable);
			...
			uint32_t idx_vf)
{
	...
	if (vf2pf_info == NULL)
		...
		if (amdgv_vfmgr_retrieve_vf2pf_message(adapt, idx_vf, vf2pf_info)) {
			AMDGV_WARN("retrieve vf2pf message failed for %s\n",
				   amdgv_idx_to_str(idx_vf));
			oss_free(vf2pf_info);
			return;
		}
	...
	if ((src != -1) && (dst != -1)) {
		...
		if (ring) {
			if (amdgv_sdma_ring_copy(ring, src, size, dst)) {
				AMDGV_ERROR("SDMA framebuffer copy failed; issuing whole GPU reset\n");
		...
		union amdgv_sched_event_data *event_data)
{
	...
	if (event_data->vf_fb_copy_data.to_fb) {
		/* Check destination when copying to framebuffer - fail if a bad page
		 * that does not exist on the source is found in the destination range
		 */
		if (amdgv_umc_dst_check_bad_pages_in_range(adapt, idx_vf, dst, size)) {
...
/*
 * Enable/disable a VF's display timer2 during live migration.
 *
 * The timer register lives in the VF's MMIO aperture, so this switches to PF,
 * temporarily grants MMIO write access (leaving it disabled afterwards), and
 * drives the timer through the irqmgr hook. On ASICs that do not register the
 * hook this is a no-op. Errors are logged and returned; callers decide whether
 * a failure is fatal to the surrounding flow.
 */
static int amdgv_sched_lm_vf_disp_timer2_control(struct amdgv_adapter *adapt, uint32_t idx_vf, bool enable)
{
	int ret;
	...
	if (ret) {
		AMDGV_ERROR("Failed to switch to PF to %s disp timer2 for %s\n",
			enable ? "enable" : "disable", amdgv_idx_to_str(idx_vf));
		return ret;
	}

	/*
	 * On export failure recovery / import the VF's MMIO write access may be
	 * disabled, so enable it here. It is always left disabled again below.
	 */
	if (!amdgv_gpuiov_get_vf_access(adapt, idx_vf, AMDGV_VF_ACCESS_MMIO_REG_WRITE)) {
		ret = amdgv_gpuiov_set_vf_access(adapt, idx_vf, AMDGV_VF_ACCESS_MMIO_REG_WRITE, true);
		if (ret) {
			AMDGV_ERROR("Failed to enable mmio write access for %s\n", amdgv_idx_to_str(idx_vf));
			return ret;
		}
	}

	ret = amdgv_irqmgr_vf_disp_timer2_control(adapt, idx_vf, enable);
	if (ret)
		AMDGV_ERROR("Failed to %s disp timer2 for %s\n",
			enable ? "enable" : "disable", amdgv_idx_to_str(idx_vf));
				       ...
				       struct amdgv_sched_event *event)
{
	...
	struct amdgv_vf_device *entry = &adapt->array_vf[event->idx_vf];
	...
	if (is_unavail_vf(event->idx_vf) &&
	    ((event->id != AMDGV_EVENT_SCHED_FORCE_RESET_GPU) &&
		 ...
		 (event->id != AMDGV_EVENT_SCHED_RAS_EVENT) &&
		 (event->id != AMDGV_EVENT_SCHED_VF_RAS_REMOTE_CMD) &&
	...
	switch (event->id) {
	case AMDGV_EVENT_SCHED_RAS_EVENT:
		amdgv_ras_event_handler(adapt, event);
		break;
	case AMDGV_EVENT_SCHED_VF_RAS_REMOTE_CMD:
		ret = amdgv_sched_handle_vf_req_ras_cmd(adapt, event);
		break;
	case AMDGV_EVENT_SCHED_RAS_UMC:
	...
	case AMDGV_EVENT_SCHED_INIT_VF_FB:
		if (is_full_access_vf(AMDGV_PF_IDX)) {
			 ...
			 */
			amdgv_vfmgr_init_vf_fb(adapt, event->idx_vf, false,
					   ...
					   struct amdgv_sched_event *event)
{
	...
	switch (event->id) {
	case AMDGV_EVENT_SCHED_VF_REQ_GPU_INIT_XCHG_REGION:
		amdgv_sched_init_xchg_region(adapt, event->idx_vf,
					    event->data.vf_xchg_region.gpa_base,
					    event->data.vf_xchg_region.size);
		break;
	case AMDGV_EVENT_REQ_GPU_INIT_DATA:
		if (adapt->flags & AMDGV_FLAG_GPUV_LIVE_MIGRATION) {
			amdgv_dirtybit_clear_fb_dbit(adapt, event->idx_vf);
		...
		amdgv_mmsch_preconfig_vf(adapt, event->idx_vf);

		/* Init PF-VF critical regions depending on highest supporting caps */
		/* Host/guest both supports GPU_CRIT_REGION_V3 */
		if (CRIT_CAP_HAS(entry->xchg.host_crit_region_caps, GPU_CRIT_REGION_V3) &&
		    CRIT_CAP_HAS(entry->xchg.guest_crit_region_caps, GPU_CRIT_REGION_V3)) {
			AMDGV_INFO("Init v3 critical region for VF%d\n", event->idx_vf);
			entry->xchg.vf_crit_region = GPU_CRIT_REGION_V3;
		/* Host/guest both supports GPU_CRIT_REGION_V2 */
		} else if (CRIT_CAP_HAS(entry->xchg.host_crit_region_caps, GPU_CRIT_REGION_V2) &&
			   CRIT_CAP_HAS(entry->xchg.guest_crit_region_caps, GPU_CRIT_REGION_V2)) {
			...
			AMDGV_INFO("Init v2 critical region for VF%d\n", event->idx_vf);
			entry->xchg.vf_crit_region = GPU_CRIT_REGION_V2;

			if (amdgv_vfmgr_init_crit_region(adapt, event->idx_vf))
				goto set_to_cond_avail;
		/* Host/guest both supports GPU_CRIT_REGION_V1 */
		} else if (CRIT_CAP_HAS(entry->xchg.host_crit_region_caps, GPU_CRIT_REGION_V1) &&
			   CRIT_CAP_HAS(entry->xchg.guest_crit_region_caps, GPU_CRIT_REGION_V1)) {
			AMDGV_INFO("Init v1 critical region for VF%d\n", event->idx_vf);
			entry->xchg.vf_crit_region = GPU_CRIT_REGION_V1;
			if (amdgv_vfmgr_init_crit_region(adapt, event->idx_vf))
				goto set_to_cond_avail;

			if (amdgv_vfmgr_check_existing_bps_in_crit_region(adapt, event->idx_vf, &has_bps))
				goto set_to_cond_avail;
			if (has_bps) {
				CRIT_CAP_CLEAR(entry->xchg.host_crit_region_caps, GPU_CRIT_REGION_V1);
				AMDGV_ERROR("Found bad page in V1 crit region on VF%d\n", event->idx_vf);
				goto set_to_cond_avail;
			}

		/* All other host/guest caps combination are unsupported so we fail the VF init */
		} else {
			AMDGV_ERROR("Guest reported critical region init capability 0x%x"
				" which host mask 0x%x does not support!\n",
				entry->xchg.guest_crit_region_caps,
				entry->xchg.host_crit_region_caps);
			goto set_to_cond_avail;
		}

		if (is_cond_avail_vf(event->idx_vf)) {
			AMDGV_INFO("Update VF%d from cond_avail to available.\n", event->idx_vf);
...
set_to_cond_avail:
	...
	case AMDGV_EVENT_SCHED_SUSPEND_VF:
		...
		if (is_active_vf(event->idx_vf)) {
			...
			/*
			 * In mmsch, calling ENABLE_AUTO after the vf is removed from activeList directly will delete the VF from mmsch.
			 * During live migration, we need to perserve the data for export.
			 * Therefore a new cmd PAUSE_VF is added to notify MMSCH to perserve the VF data.
			 * currently only send this cmd during LM. If there are other cases suspendVF is called for virtual function,
			 * we need to consider consider expending the condition.
			 */
			 if (adapt->flags & AMDGV_FLAG_USE_PF &&
				adapt->live_migration.mig_state[event->idx_vf].state == AMDGV_MIGRATION_VF_STATE_EXPORT) {
				AMDGV_INFO("VF %s is in live migration export, pause VF\n", amdgv_idx_to_str(event->idx_vf));

				/* no-op on ASICs that do not register the disp timer2 hook */
				amdgv_sched_lm_vf_disp_timer2_control(adapt, event->idx_vf, false);

				amdgv_sched_pause_vf(adapt, event->idx_vf);
			}
			amdgv_sched_remove_vf(adapt, event->idx_vf);
		} else {
	...
	case AMDGV_EVENT_SCHED_RESUME_VF:
		if ((adapt->flags & AMDGV_FLAG_GPUV_LIVE_UPDATE) && (adapt->live_update_state == AMDGV_LIVE_UPDATE_SAVE)) {
			...
			if (adapt->irqmgr.vf_disp_timer2_control &&
				(adapt->live_migration.mig_state[event->idx_vf].state == AMDGV_MIGRATION_VF_STATE_IMPORT ||
				adapt->live_migration.mig_state[event->idx_vf].state == AMDGV_MIGRATION_VF_STATE_EXPORT)) {
				AMDGV_INFO("VF %s is in live migration %s, enable disp timer2\n", amdgv_idx_to_str(event->idx_vf),
					adapt->live_migration.mig_state[event->idx_vf].state == AMDGV_MIGRATION_VF_STATE_IMPORT ? "import" : "export");

				amdgv_sched_lm_vf_disp_timer2_control(adapt, event->idx_vf, true);
			}
			...
			if (AMDGV_PF_IDX != event->idx_vf) {
				...
				/* migrated target: apply rb_decouple now that the VF is
				 * active and before world switch resumes. is_target is set
				 * in shared IMPORT_PREPARE, so this works for both GIM/KVM
				 * (never sets mig_state IMPORT) and msgpuv */
				if (adapt->live_migration.mig_state[event->idx_vf].is_target) {
					AMDGV_INFO("VF %s migration import: applying vf2pf rb_decouple config before world switch resume\n",
						   amdgv_idx_to_str(event->idx_vf));
					amdgv_sched_check_vf2pf_data(adapt, event->idx_vf);

					/* import is finalized only when the VF is resumed;
					 * mark it initialized here so a failure in a later import
					 * step (e.g. FB copy) doesn't leave a half-migrated VF
					 * flagged as ready */
					adapt->array_vf[event->idx_vf].vf_status = AMDGV_VF_STATUS_END_INIT;
				}
			}
	...
	case AMDGV_EVENT_LIVE_MIGRATION_MANIFEST_DATA:
		...
		if (amdgv_sched_active_vf_ex_pf_num(adapt) > 0 ||
			((adapt->flags & AMDGV_FLAG_USE_PF) &&
			(event->data.lm.type == AMDGV_MIGRATION_IMPORT_PREPARE ||
	...
	case AMDGV_EVENT_VF_FB_COPY_ASYNC:
	{
		int copy_ret = 0;
		uint32_t i;
		bool to_fb = event->data.vf_fb_copy_data.to_fb;
		...
		for (i = 0; i < event->data.vf_fb_copy_data.num_entries; i++) {
			...
			struct oss_dma_mem_info *dma_mem =
				(struct oss_dma_mem_info *)entry->dma_handle;
			uint64_t buf_off;

			if (!AMDGV_FB_COPY_ENTRY_FLUSHABLE(dma_mem, entry)) {
				copy_ret = AMDGV_FAILURE;
				break;
			}
			buf_off = (uint64_t)((char *)entry->vaddr - (char *)dma_mem->va_ptr);
			...
			/* to_fb copy reads the sysmem buffer the OS just wrote:
			 * sync CPU writes before SDMA reads this range. Done in
			 * the event thread so it overlaps the OS stream caller. */
			if (to_fb)
				oss_flush_dma_mem(dma_mem, OSS_DMA_DIR_FROM_MEMORY,
						  buf_off, entry->size);
			...
			/* from_fb copy wrote the sysmem buffer via SDMA: sync
			 * those writes before the OS reads this range. */
			if (!to_fb)
				oss_flush_dma_mem(dma_mem, OSS_DMA_DIR_TO_MEMORY,
						  buf_off, entry->size);
		}
	...
	case AMDGV_EVENT_QUERY_DIRTYBIT_DATA:
		...
		*event->data.dirtybit_query_data.result =
			amdgv_dirtybit_query_and_accumulate(adapt, &event->data.dirtybit_query_data.data);
	...
	case AMDGV_EVENT_SET_VF_MIGRATION_STATE:
		*event->data.migration_state.result =
		...
		if (adapt->flags & AMDGV_FLAG_USE_PF) {
			uint32_t pf_timeslice = GET_GFX_TIME_SLICE(adapt,
					adapt->sched.num_vf_per_gfx_sched, AMDGV_PF_IDX);

			AMDGV_INFO("Live migration: PF gfx timeslice = %u us\n", pf_timeslice);
			amdgv_sched_setup_vf_timeslice(adapt, AMDGV_PF_IDX, pf_timeslice,
						     AMDGV_SCHED_BLOCK_GFX);
		}
	...
	case AMDGV_EVENT_SCHED_SET_VF_COND_AVAIL:
		...
		break;
	case AMDGV_EVENT_SCHED_UAL_PAUSE_REQ:
		ret = amdgv_ual_pause(adapt, true);
		break;
	case AMDGV_EVENT_SCHED_UAL_RESUME_REQ:
...
static int amdgv_sched_event_process_thread(void *context)
{
	...
	oss_mutex_unlock(adapt->sched.event_thread_lock);
	AMDGV_DEBUG("event process thread exiting!\n");
					    ...
					    union amdgv_sched_event_data data)
{
	...
	if (amdgv_sched_is_unrecov_err(adapt) &&
		...
		AMDGV_ERROR("dropping event %s from %s: adapter in unrecoverable error state\n",
			    amdgv_event_name(event_id), amdgv_idx_to_str(idx_vf));
		return AMDGV_FAILURE;
	}
	...
	if (AMDGV_EVENT_REQ_GPU_INIT <= event_id &&
		...
		if (event_id == AMDGV_EVENT_REQ_GPU_INIT ||
			...
			if (ret == AMDGV_EVENT_OVERFLOW) {
				amdgv_put_log(idx_vf, AMDGV_LOG_SCHED_IGNORE_EVENT,
						event_id);
				return AMDGV_FAILURE;
			}
		...
		if (amdgv_guard_is_event_full(adapt, idx_vf, AMDGV_GUARD_EVENT_FLR) ==
			...
			amdgv_put_log(idx_vf, AMDGV_LOG_GUARD_EVENT_OVERFLOW, event_id);
			return AMDGV_FAILURE;
		}
...
int amdgv_sched_event_queue_process_init(struct amdgv_adapter *adapt)
{
	...
	AMDGV_DEBUG("allowed time for full access is %dms\n",
		   adapt->sched.allow_time_full_access / 1000);
					      ...
					      uint32_t *unprocessed_event_count)
{
	...
	if (unprocessed_event_count == NULL) {
		AMDGV_ERROR("unprocessed_event_count is NULL!\n");
		return AMDGV_FAILURE;
	}
	...
	*unprocessed_event_count = 0;
	...
	AMDGV_DEBUG("Unprocessed event count is %d.\n", *unprocessed_event_count);
					 ...
					 struct amdgv_live_info_unprocessed_event *event_info)
{
	...
	if (event_info == NULL) {
		AMDGV_ERROR("event_info is NULL!\n");
	 ...
	 */
	while ((event = amdgv_sched_pick_up_next_event(adapt)) != NULL) {
		AMDGV_DEBUG("save %s request from %s for %s\n", amdgv_event_name(event->id),
			   amdgv_idx_to_str(event->idx_vf),
			   amdgv_sched_block_to_name(event->sched_block));
		if (event->signal != OSS_INVALID_HANDLE)
			oss_signal_event(event->signal);
	...
	if (!amdgv_sched_event_queue_empty(adapt))
		AMDGV_WARN("New events detected after saving!\n");
