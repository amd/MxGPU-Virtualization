...
#define FEATURE_ID_SPARE_30               30
#define FEATURE_ID_GFX_DC_RTC             31
#define FEATURE_ID_SPARE_32               32
...
#define SMU_METRICS_TABLE_VERSION 0xF//NOT CURRENTLY USED BY DRIVER, BUT INCREMENT FOR ANY UPDATES
...
typedef struct {
  ...
  uint32_t HbmTemperature[12]; //per-stack HBM temperature
  uint64_t MaxSocketTemperatureAcc;
  uint64_t MaxVrTemperatureAcc;
  uint64_t HbmTemperatureAcc[12]; //per-stack HBM temperature
  ...
  uint32_t FclkFrequency[2]; //1 per aid
  uint32_t UclkFrequency[2]; //1 per aid
  uint64_t GfxclkFrequencyAcc[8]; //1 per xcd
  uint32_t GfxclkFrequency[8]; //1 per xcd
  uint32_t SocclkFrequency[2]; //1 per mid
  uint32_t VclkFrequency[4];   //2 per mid
  uint32_t DclkFrequency[4];   //2 per mid
  uint32_t LclkFrequency[2];   //1 per mid
  ...
  uint64_t PcieBandwidthAcc[2]; // 1 per mid
  ...
  // FIX ME AM: Controversial
  // uint32_t GfxLockXCDMak;

  //PCIE BW Data and error count
  uint32_t PcieBandwidth[2]; //1 per mid
  uint64_t PCIeL0ToRecoveryCountAcc;      // Accumulated, only host connected PCIE port metrics
  uint64_t PCIenReplayAAcc;               // Accumulated, only host connected PCIE port metrics
  uint64_t PCIenReplayARolloverCountAcc;  // Accumulated, only host connected PCIE port metrics
  uint64_t PCIeNAKSentCountAcc;           // Accumulated, only host connected PCIE port metrics
  uint64_t PCIeNAKReceivedCountAcc;       // Accumulated, only host connected PCIE port metrics
  uint64_t PCIeOtherEndRecoveryAcc;       // The Pcie counter itself is accumulated

  // VCN/JPEG ACTIVITY
  uint32_t VcnBusy[4]; //2 per mid
  uint32_t JpegBusy[40]; // 20 per mid

  // PCIE LINK Speed and width
  uint32_t PCIeLinkSpeed; // Accumulated, only host connected PCIE port metrics
  uint32_t PCIeLinkWidth; // Accumulated, only host connected PCIE port metrics
...
#pragma pack(push, 4)
typedef struct {
  uint64_t AccumulationCounter;                             // Last update timestamp, last time an APML value was updated
                                                            // Downstream tools can use this to tell if data is stale.
  uint16_t LabelVersion;                                    // If SMC label mapping changes this needs to be updated. Defaults to 0.
  uint16_t NodeIdentifier;                                  // SMC Pushes a unique identifier to each node on system. For debug purposes.
...
typedef struct {
  ...
  uint32_t SocclkFrequency;//boot-time setting, pre-DS
  uint32_t LclkFrequency;//boot-time setting, pre-DS
  uint32_t VclkFrequency;//boot-time setting, pre-DS
  uint32_t DclkFrequency;//boot-time setting, pre-DS
  ...
  // Telemetry
  uint32_t InputTelemetryVoltageInmV;  // AM FIXME

  // General info
  uint32_t pldmVersion[2]; //AM: I2c Read

  // FWDEV-155179 PPT1 Configuration
  uint32_t PPT1Max;
  uint32_t PPT1Min;
